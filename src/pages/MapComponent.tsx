...
export default function MapComponent() {
  ...
  const mapFunction = (mapType:string) => {
    var esriConfig = {
      portalUrl: "https://loaylq04mkqd09ex.esri.com/arcgis"
    };
    const map = new Map({
      basemap: 'hybrid',
    });
    ...
    const trailsRendererForKoordinates = {
      type: "unique-value",
      valueExpression: "IIf(Find('#', $feature.PropAddr10) > -1, 'yellow', 'default')",
      uniqueValueInfos: [
        {
          value: 'yellow',
          symbol: {
    ...
    if(mapType == 'Parcel_View' || mapType == 'Income_Centroids' || mapType == 'Income_Boundaries' || (mapType == 'Client_Data' && kmlUrl)){
      if(mapType == 'Client_Data'){
        featureLayer = new FeatureLayer({
          url: featureURL,
          // @ts-ignore
      ...
      }else if(mapType == 'Parcel_View') {
        featureLayer = new FeatureLayer({
          url: featureURL,
          // @ts-ignore
          renderer:trailsRendererForKoordinates,
          opacity: 1
        });
      }
      else {
      ...
      .when(() => {
        ...
        mapView.on('click', async (event) => {
          ...
          try {
            ...
            const parcelData = await getSelectedData(mapType!=="Client_Data"? "Parcel_View": "Client_Data", event.mapPoint, 2);
            ...
            if(mapType == 'Client_Data'){
              // const regridAPIendpoint = `https://app.regrid.com/api/v2/parcels/point?lat=${event.mapPoint.latitude}&lon=${event.mapPoint.longitude}&token=Nl1E4Rkk1GyTq1RK7TkUq5J6wfHwh0h4DygFYxdUsfQl965F9VWz8tqiwb4xcyOF`
              // const parcelFromRegrid = await axios.get(regridAPIendpoint)
              // console.log("=======regridAPIendpoint=====", regridAPIendpoint)
              // console.log("=======parcelFromRegrid=====", parcelFromRegrid)
              // setDisplayData({elevation:elevationResult});
              // setVisible(true);
            }
    ...
    <section id="map-page-container" className="w-full h-screen">
      ...
      >
        ...
        <Sidebar visible={visible} onHide={() => setVisible(false)}>
          ...
          {displayData && selectedMap == "Client_Data" && kmlUrl &&
            <div style={{lineHeight:'30px', marginLeft:'15px', fontWeight:'400'}}>
              <p><span style={{fontWeight:'500'}}>Owner:</span>&nbsp; {displayData.Owner}</p>
              <p><span style={{fontWeight:'500'}}>Owner2:</span>&nbsp; {displayData.Owner2}</p>
              <p><span style={{fontWeight:'500'}}>STLength:</span>&nbsp; {displayData.STLength.toFixed(2)}</p>
              <p><span style={{fontWeight:'500'}}>Date_Updated:</span>&nbsp; {new Date(displayData.Date_Updat).toLocaleString()}</p>
              <p><span style={{fontWeight:'500'}}>Acres:</span>&nbsp; {displayData.Acres}</p>
              <p><span style={{fontWeight:'500'}}>PropAddress_Full:</span>&nbsp; {displayData.PropAddr10}</p>
              <p><span style={{fontWeight:'500'}}>PropAddress_City:</span>&nbsp; {displayData.PropAddr_6}</p>
              <p><span style={{fontWeight:'500'}}>PropAddress_State:</span>&nbsp; {displayData.PropAddr_1}</p>
              <p><span style={{fontWeight:'500'}}>PropAddress_StreetName:</span>&nbsp; {displayData.PropAddres}</p>
              <p><span style={{fontWeight:'500'}}>PropAddress_StreetType:</span>&nbsp; {displayData.PropAddr_8}</p>
              <p><span style={{fontWeight:'500'}}>PropAddress_Zip:</span>&nbsp; {displayData.PropAddr_5}</p>
              <p><span style={{fontWeight:'500'}}>PIN:</span>&nbsp; {displayData.PIN}</p>
              <p><span style={{fontWeight:'500'}}>Mailing_Address:</span>&nbsp; {displayData.Mailing_Ad}</p>
              <p><span style={{fontWeight:'500'}}>Mailing_City:</span>&nbsp; {displayData.Mailing_Ci}</p>
              <p><span style={{fontWeight:'500'}}>Mailing_State:</span>&nbsp; {displayData.Mailing_St}</p>
              <p><span style={{fontWeight:'500'}}>Mailing_Zip:</span>&nbsp; {displayData.Mailing_Zi}</p>
              <p><span style={{fontWeight:'500'}}>Land_Use_1:</span>&nbsp; {displayData.Land_Use_1}</p>
              <p><span style={{fontWeight:'500'}}>Land_Use_2:</span>&nbsp; {displayData.Land_Use_2}</p>
              <p><span style={{fontWeight:'500'}}>Land_Use_3:</span>&nbsp; {displayData.Land_Use_3}</p>
              <p><span style={{fontWeight:'500'}}>Land_Use_4:</span>&nbsp; {displayData.Land_Use_4}</p>
              <p><span style={{fontWeight:'500'}}>Sale_Price:</span>&nbsp; {displayData.Sale_Price}</p>
              <p><span style={{fontWeight:'500'}}>Sale_MM:</span>&nbsp; {displayData.Sale_MM}</p>
              <p><span style={{fontWeight:'500'}}>Sale_DD:</span>&nbsp; {displayData.Sale_DD}</p>
              <p><span style={{fontWeight:'500'}}>Sale_YY:</span>&nbsp; {displayData.Sale_YY}</p>
              <p><span style={{fontWeight:'500'}}>Sale_Quality:</span>&nbsp; {displayData.Sale_Quali}</p>
              <p><span style={{fontWeight:'500'}}>STArea:</span>&nbsp; {displayData.STArea.toFixed(2)}</p>
              <p><span style={{fontWeight:'500'}}>Shape__Area:</span>&nbsp; {displayData.Shape__Area.toFixed(2)}</p>
              <p><span style={{fontWeight:'500'}}>Shape__Length:</span>&nbsp; {displayData.Shape__Length.toFixed(2)}</p>
              <p><span style={{fontWeight:'500'}}>YearBuilt:</span>&nbsp; {displayData.YearBuilt}</p>
              <p><span style={{fontWeight:'500'}}>Legal_Description:</span>&nbsp; {displayData.Legal_Desc}</p>
              <p><span style={{fontWeight:'500'}}>Municipality:</span>&nbsp; {displayData.Municipali}</p>
              <p><span style={{fontWeight:'500'}}>District_DryTrash:</span>&nbsp; {displayData.District_D}</p>
              <p><span style={{fontWeight:'500'}}>District_TAD:</span>&nbsp; {displayData.District_T}</p>
              <p><span style={{fontWeight:'500'}}>District_Transit:</span>&nbsp; {displayData.District_1}</p>
              <p><span style={{fontWeight:'500'}}>District_EnterpriseZone:</span>&nbsp; {displayData.District_E}</p>
              <p><span style={{fontWeight:'500'}}>Commercial_Cat:</span>&nbsp; {displayData.Commercial}</p>
              <p><span style={{fontWeight:'500'}}>Property_Use:</span>&nbsp; {displayData.Property_U}</p>
            </div>
          }
